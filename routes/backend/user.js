...
userRouter.get('/edit/:id',login.authorization,async function(req,res){
    if(req.user){
        await user.getPage(req,res)
    }else{
        res.redirect('/admin/login')
...
userRouter.post('/edit/:id',login.authorization,async function(req,res){
    if(req.user){
...
userRouter.get('/delete/:id',login.authAdmin,async function(req,res){
    if(req.userRole){
